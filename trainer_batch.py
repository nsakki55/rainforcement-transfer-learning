...
class Trainer:
    ...
    def train(self):
        ...
        policy_batch, log_probs, entropies = self.controller.sample_batch(self.args.policy_batch_size, with_details=True)
        ...
        for self.epoch in range(self.start_epoch, self.args.max_epoch):
            ...
            for self.train_step, (inputs, labels) in enumerate(self.train_data):
                ...
                policy = policy_batch[policy_num].cpu().numpy()
                optimizer = self.set_lr_optimizer(self.shared, self.shared_optim, policy)        
                optimizer.zero_grad()
                ...
                val_acc, val_loss = self.get_validation_acc(self.shared)
                policy_val_acc.append(val_acc)
                policy_val_loss.append(val_loss)
                policy_num += 1
                if (self.train_step + 1) % self.args.policy_batch_size == 0:
                    logger.info('policy:{}'.format(policy))
                    utils.log_optimizer_lr(optimizer)
                    train_loss_avg = train_loss.item() / self.args.policy_batch_size
                    train_acc = train_correct / train_count
                    val_loss_avg = np.mean(policy_val_loss)
                    val_acc_avg =  np.mean(policy_val_acc)
                    self._summarize_shared_train(policy, train_loss_avg, train_acc, val_loss_avg, val_acc_avg)

                    self.train_controller_batch(policy_val_acc, log_probs, entropies)
    ...
    def train_controller_batch(self, val_acc, log_probs, entropies):
        self.controller.train()
        self.shared.eval()

        # policyごとのエントロピーの平均
        np_entropies = entropies.cpu().detach().numpy()
        
        np_val_acc = np.array(val_acc)

        # 各policyに対して報酬を求める
        rewards = np_val_acc + self.args.entropy_coeff * np_entropies
        ...
        if self.baseline is None:
            ...
            self.baseline = decay * self.baseline + (1 - decay) * mean_rewards
        ...
        self._summarize_controller_train(np_loss, adv, rewards, np_entropies, self.baseline)
